...
const watchFiles = [
  {
    ...
    resolve(item, config) {
      ...
      if (resolversDirectory && resolversDirectory.children) {
        ...
        graphqlConfig.resolvers = resolver ? path.resolve(config.path, resolver.path) : null
      }
...
function parseComponents(components = [], rootPath) {
  ...
  */
  return components.reduce((parsedComponents, item) => {
    let component
    let componentName = item.name.replace(/\..*$/, '')
    if (item.type === 'file') {
...
module.exports = function(dir) {
  ...
  parseItems(tree.children.filter(child => child.name === 'src'), config)
