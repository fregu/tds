...
async function createServer({
  ...
  const context = {
    ...
    assets:
      (distManifest &&
        Object.keys(distManifest).map(key =>
          distManifest[key].replace(/^\//, '')
        )) ||
      []
  }
  ...
  console.log(resolvers, typeDefs, remoteSchemas)
  ...
  app.use((ctx, next) => {
    console.log('is asset?', ctx.path, context.assets, publicPath)
  ...
  return {
    ...
    async update(buildAssets) {
      ...
      } else if (buildAssets.find(file => file.match(/\.(s?css|styl|less)$/))) {
