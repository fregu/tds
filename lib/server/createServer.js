...
async function createServer(
  {
    ...
    ssr: enableSSR = true,
    paths: { root: rootPath = process.cwd(), dist: distPath = 'dist' } = {},
    graphql: {
      resolvers,
      typeDefs,
      schema,
  ...
  app.use((ctx, next) => {
    ...
    if (
      ...
      ctx.status = 200
      const fontMatch = assetMatch.match(/\.(woff2?)$/)
      ctx.type = fontMatch
        ? `application/font-${fontMatch[1]}`
        : mime.contentType(assetMatch)
  ...
  if (keys.bugsnag && mode === 'production') {
  ...
  if (keys.sentry && mode === 'production') {
  ...
  const apollo = new ApolloServer({
    ...
    context: ({ ctx }) => ctx,
    formatError: error => {
      console.log('GraphQL formatError', error)
      return error
    },
    introspection: true,
    playground: isDev && {
  ...
  app.use(async (ctx, next) => {
    if (notReadyBrowser) {
      console.log(
        chalk.blue('[TDS]:'),
        chalk.green('Wait until bundle is ready')
      )
  ...
  app.use(mount('/static', serve(path.resolve(rootPath, 'static'))))
