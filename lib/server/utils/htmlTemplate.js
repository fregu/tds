export default function htmlTemplate(
  ...
  const stateScript = `
    <script>
      window.__REDUX_STATE__ = "${encodeURIComponent(
        JSON.stringify(reduxState)
      )}"
      window.__APOLLO_STATE__ = "${encodeURIComponent(
        JSON.stringify(apolloState)
      )}"
      ${
        graphql.endpoint
  ...
  return html
    .replace(
      '<div id="root"></div>',
      `${(helmetData && helmetData.noscript.toString()) ||
        ''}<div id="root">${reactDom}</div>${stateScript}`
    )
    .replace('</head>', `${headString}</head>`)
    .replace(
      '</head>',
      `${(helmetData && helmetData.script.toString()) || ''}</head>`
    )
    .replace(
      '<body',
