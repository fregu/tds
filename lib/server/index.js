...
// DEPRECATED, but keep for reference to make sure..
...
async function createServer(config) {
  ...
  app.use(async (ctx, next) => {
    if (notReadyBrowser || (notReadyStyleguide && ctx.path.match(/\.html/))) {
  ...
  if (config.graphql) {
    config.graphqlEndpoint = `http://${host}${port ? `:${port}` : ''}/graphql`
