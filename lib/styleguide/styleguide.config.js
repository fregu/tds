...
module.exports = {
  ...
  sections: [{
    name: 'Dokumentation',
    description: 'Riktlinjer och guidelines för användningen av Riskbyggens UI komponenter',
  ...
  updateExample(props, exampleFilePath) {
    ...
    if (typeof settings.cssvar === 'string') {
      ...
      // no longer needed
      delete settings.file
      const customProperties = fs.readFileSync(filepath, 'utf8').match(/(--\S*: .*;)\n*/g)
    ...
    if (typeof settings.cssimport === 'string') {
      ...
      // no longer needed
      delete settings.file
      const imports = fs.readFileSync(filepath, 'utf8').match(/(@import.*;)\n*/g)
